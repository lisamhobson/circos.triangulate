...
circos_protein_plot_segment <- function(circos_data,
  ...
  circos.par("track.height"= total_track_number, gap.degree = 270, start.degree=90, cell.padding=c(0, 0, 0, 0), canvas.xlim = c(0,1.05), canvas.ylim=c(0, 1), circle.margin=rep(1, 4))
  circos.initialize(factors=circos_data_track_main_segment$tier_section, xlim=matrix(c(rep(0, nrow(freq_table_segment)), freq_table_segment$`length(tier_section)`), ncol=2))
