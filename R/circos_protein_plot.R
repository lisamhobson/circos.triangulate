...
circos_protein_plot <- function(circos_data,
  ...
  if(missing(primary_track)) {
    track_factors <- as.factor(circos_data$track_id) %>% levels()
    primary_track <- track_factors[1]
  } else {
    primary_track <- as.factor(primary_track)
  }

  circos_data <- circos_data %>% mutate(track_id_factor = factor(track_id))
  ...
  circos_data_track_main$x <- with(circos_data_track_main %>% filter(track_id == primary_track), ave(seq_along(circos_data_track_main[[segment_names_column]]),
  ...
  for (i in 1:nrow(circos_data_track_main)) {
    circos_data_track_main$n[i] <- as.numeric(nrow(subset(circos_data_track_main, circos_data_track_main[[segment_names_column]] == circos_data_track_main[[segment_names_column]][i])))
    ...
    circlize::circos.track(factors = circos_data$tier_section,
                           ...
                           track.height = 0.6/total_track_number, panel.fun = function(x, y) {
                             ...
                             circlize::circos.points(
                               ...
                               y=get(paste0("circos_data_track_", i)) %>% filter(tier_section == circlize::get.cell.meta.data("sector.index")) %>% pull(y_value),
                               pch = 16,
                               cex = .5,
                               col = colour_contrast_checker(custom_palette[i]))

                             circlize::circos.segments(
                               x0 = get(paste0("circos_data_track_", i)) %>% filter(tier_section == circlize::get.cell.meta.data("sector.index")) %>% pull(x) -0.5,
                               y0 = get(paste0("circos_data_track_", i)) %>% filter(tier_section == circlize::get.cell.meta.data("sector.index")) %>% pull(lo_ci95),
                               x1 = get(paste0("circos_data_track_", i)) %>% filter(tier_section == circlize::get.cell.meta.data("sector.index")) %>% pull(x) -0.5,
                               y1 = get(paste0("circos_data_track_", i)) %>% filter(tier_section == circlize::get.cell.meta.data("sector.index")) %>% pull(up_ci95),
                               col = colour_contrast_checker(custom_palette[i]),
                               straight = T)
                             ...
                             if(error_bar_ends == T) {
                               circlize::circos.segments(
                                 y0 = get(paste0("circos_data_track_", i)) %>% filter(tier_section == circlize::get.cell.meta.data("sector.index")) %>% pull(lo_ci95),
                                 x0 = get(paste0("circos_data_track_", i)) %>% filter(tier_section == circlize::get.cell.meta.data("sector.index")) %>% pull(x) - 0.5 +0.05,
                                 y1 = get(paste0("circos_data_track_", i)) %>% filter(tier_section == circlize::get.cell.meta.data("sector.index")) %>% pull(lo_ci95),
                                 x1 = get(paste0("circos_data_track_", i)) %>% filter(tier_section == circlize::get.cell.meta.data("sector.index")) %>% pull(x) - 0.5 -0.05,
                                 col = colour_contrast_checker(custom_palette[i]),
                                 straight = T)

                               circlize::circos.segments(
                                 y0 = get(paste0("circos_data_track_", i)) %>% filter(tier_section == circlize::get.cell.meta.data("sector.index")) %>% pull(up_ci95),
                                 x0 = get(paste0("circos_data_track_", i)) %>% filter(tier_section == circlize::get.cell.meta.data("sector.index")) %>% pull(x) - 0.5 +0.05,
                                 y1 = get(paste0("circos_data_track_", i)) %>% filter(tier_section == circlize::get.cell.meta.data("sector.index")) %>% pull(up_ci95),
                                 x1 = get(paste0("circos_data_track_", i)) %>% filter(tier_section == circlize::get.cell.meta.data("sector.index")) %>% pull(x) - 0.5 -0.05,
                                 col = colour_contrast_checker(custom_palette[i]),
